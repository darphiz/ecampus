        ...
        if q is None or q is "":
            ...
            if request.user.is_authenticated:
                try:
                    ...
                    faculty_questions = Question.Approved.filter(faculty = faculty).order_by('?')[:15]
            ...
            paginator = Paginator(question_list, 4) #displays 7 questions per page
            page = request.GET.get('page')
            questions = paginator.get_page(page)
        return render(request,
            'ask.html',
...
#paginator
    ...
    return render(request,
    'question_progress.html', {'questions': questions, 'unapproved': unapproved}
)