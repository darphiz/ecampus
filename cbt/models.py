    ...
    def list_all_cat_scores(self):
        ...
        for cat in Category.objects.all():
            ...
            if match:
                ...
                try:
                    percent = float( ((float(score) / float(possible))
                                        * 5.0))
                except:
                    percent = 0
...
class Sitting(models.Model):
    ...
    def get_question_number(self,question_number):      
        ...
        q_list = self.question_order.split(',')
        q_number = int(question_number-1)
    ...
    def remove_from_wrong(self,question):
        incorrect = self.get_incorrect_questions
        incorr = incorrect.remove(question.id)
        self.incorrect_questions = ','.join(map(str, incorr))
        self.save()
